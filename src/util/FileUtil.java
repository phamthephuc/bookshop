...
public final class FileUtil {
	...
	public static void rePlace(CommonsMultipartFile cmf, HttpServletRequest request,String oldFileName) throws IOException {
		fileName = cmf.getOriginalFilename();
		String appPath = request.getServletContext().getRealPath("");
		String dirPath = appPath + Defines.DIR_UPLOAD;
		...
		if (!"".equals(fileName)) {
			...
			try {
				cmf.transferTo(new File(filePath));
				if(!"".equals(oldFileName))delete(oldFileName, request);
			} catch (IOException e) {
				throw new IOException();
			}
		}
